...
CMD /prod/api/target/realse/api -- --api-url=${API_URL} --biscuit-private-key=${BISCUIT_PRIVATE_KEY} --database-url=${DATABASE_URL} --email-sender-address=${EMAIL_SENDER_ADDRESS} --smtp-connection-url=${SMTP_CONNECTION_URL}