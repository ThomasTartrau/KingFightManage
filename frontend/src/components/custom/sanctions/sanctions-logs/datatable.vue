...
function onSearch(search: string) {
  datas.value = props.data.filter(
    (sanction) =>
      sanction.player_id.includes(search) ||
      sanction.player_name.includes(search) ||
      sanction.staff_name.includes(search),
  );
}
  ...
  <div class="w-full">
    <div class="relative w-full max-w-sm items-center mb-8">
      <Input
        id="search"
        type="text"
        placeholder="Rechercher un id ou nom de joueur ou un nom de staff"
